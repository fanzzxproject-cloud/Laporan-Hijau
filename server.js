...
app.post('/api/laporan/sync', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('laporan')
    .select('id, status, balasan, foto_dokumentasi')
    .in('id', ids);
...
// Leaderboard kelas berdasarkan poin
app.get('/api/leaderboard', async (req, res) => {
  ...
  const kelasMap = {};
  data.forEach(l => {
    if (!kelasMap[l.kelas]) kelasMap[l.kelas] = { kelas: l.kelas, total_laporan: 0, poin: 0 };
    kelasMap[l.kelas].total_laporan++;
    kelasMap[l.kelas].poin += l.status==='selesai' ? 10 : l.status==='diproses' ? 5 : 2;
  });

  const result = Object.values(kelasMap).sort((a,b) => b.poin - a.poin);
